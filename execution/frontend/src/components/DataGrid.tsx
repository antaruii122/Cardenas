                ...
                <Table>
                    <TableHeader className="bg-white/5 sticky top-0 backdrop-blur-md z-10">
                        <TableRow className="hover:bg-transparent border-white/10">
                            <TableHead className="text-gray-300 w-[120px]">Fila Excel</TableHead>
                            <TableHead className="text-gray-300">Ítem Detectado</TableHead>
                            <TableHead className="text-right text-gray-300">Valores Extraídos</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {rawRows.map((row, i) => (
                            <TableRow key={i} className="hover:bg-white/5 border-white/5">
                                ...
                                <TableCell className="text-right font-mono text-emerald-400">
                                    {Object.entries(row.values).map(([period, value]) => (
                                        <div key={period} className="text-xs flex justify-end gap-2">
                                            <span className="text-gray-500">{period}:</span>
                                            <span>{new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP' }).format(value)}</span>
                                        </div>
                                    ))}
                                </TableCell>
                            </TableRow>
